...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      'admin_bar_menu_icon'     => 'dashicons-welcome-learn-more',
      'admin_bar_menu_priority' => 50,

      // footer
      'footer_text'             => 'ممنون از اینکه این افزونه رو برای کسب و کار خودتون انتخاب کردید',
      'footer_after'            => '',
      'footer_credit'           => '',
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'تغیرات به حالت اولیه برگشت داده شد.', 'csf' );
        ...
        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'تغیرات ذخیره شد.', 'csf' );
        }
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'یک سری دیتا باقی مونده که ذخیره نکردی یادت نره ذخیره کنی', 'csf' ) .'</div>' : '';



            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'ذخیره', 'csf' ) .'" data-save="'. esc_html__( 'در حال ذخیره سازی تغیرات ...', 'csf' ) .'">';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'ریست کردن همه', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'مطمعنی میخوای از اول شروع کنی؟', 'csf' ) .'">' : '';
            echo '</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'ذخیره', 'csf' ) .'" data-save="'. esc_html__( 'Saving...', 'csf' ) .'">';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'ریست کردن همه', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'مطمعنی که میخوای از اول شروع کنی؟', 'csf' ) .'">' : '';
          echo '</div>';
